...
RESTAURANTS = [
    {
        ...
        "operatingHours": {
            ...
            "wednesday": "09:00-23:00", "thursday": "09:00-23:00",
            "friday": "09:00-23:00", "saturday": "10:00-00:00",
            "sunday": "10:00-22:00",
        },
    },
    {
        ...
        "operatingHours": {
            "monday": "09:00-23:00", "tuesday": "09:00-23:00",
            "wednesday": "09:00-23:00", "thursday": "09:00-23:00",
            "friday": "14:00-00:00", "saturday": "10:00-00:00",
            "sunday": "10:00-22:00",
        },
    },
    {
        ...
        "operatingHours": {
            ...
            "wednesday": "10:00-22:00", "thursday": "10:00-23:00",
    ...
    },
    {
        ...
        "operatingHours": {
            "monday": "12:00-23:00", "tuesday": "12:00-23:00",
            "wednesday": "12:00-23:00", "thursday": "12:00-00:00",
            "friday": "12:00-00:00", "saturday": "10:00-00:00",
            "sunday": "10:00-22:00",
        },
    },
]
...
USERS = {
    "admin": {
        "email": "admin@mayda.dz", "phone": 700000001,
        "firstName": "Yasmine", "lastName": "Mansouri",
        "password": "admin123456", "role": "ADMIN",
    },
    "managers": [
        {"firstName": "Karim", "lastName": "Benaïssa", "email": "karim.benaissa@mayda.dz", "phone": 711111111, "ri": 0},
        {"firstName": "Nadia", "lastName": "Toumi", "email": "nadia.toumi@mayda.dz", "phone": 711111112, "ri": 1},
        {"firstName": "Sami", "lastName": "Guerroudj", "email": "sami.guerroudj@mayda.dz", "phone": 711111113, "ri": 2},
        {"firstName": "Inès", "lastName": "Boukhemis", "email": "ines.boukhemis@mayda.dz", "phone": 711111114, "ri": 3},
    ],
    "staff": [
    ...
    "clients": [
        ...
        {"firstName": "Ilyes", "lastName": "Tahar", "email": "ilyes.tahar@email.com", "phone": 733333337},
        {"firstName": "Sofia", "lastName": "Chennouf", "email": "sofia.chennouf@email.com", "phone": 733333338},
    ],
}
...
DISHES = {  # each entry: (name, description, price_dzd, prep_min, popularity)
    "Entrées": [
        ("Chorba Frik", "Soupe traditionnelle algérienne au frik (blé vert concassé), tomates, pois chiches et viande d'agneau", 4.50, 15, 4.8),
        ("Chorba Beïda", "Soupe blanche au poulet, cannelle et citron, spécialité constantinoise", 4.00, 12, 4.6),
        ("Harira", "Soupe algérienne riche en tomates, pois chiches, lentilles et coriandre", 4.50, 20, 4.7),
        ("Chourba Djedj", "Soupe de poulet aux vermicelles et légumes frais", 3.50, 12, 4.5),
        ("Bourek Annabi", "Brick à la viande hachée, oignons et persil, spécialité d'Annaba", 5.00, 10, 4.9),
        ("Mhadjeb", "Crêpe feuilletée farcie aux oignons, tomates et poivrons", 3.00, 8, 4.6),
        ("Karantika", "Flan de pois chiches cuit au four, spécialité oranaise", 2.50, 5, 4.5),
        ("Tadjine Malsouf", "Omelette aux légumes et à la viande hachée", 4.00, 12, 4.4),
    ],
    "Salades": [
        ("Salade Mechouia", "Salade de poivrons grillés, tomates, oignons et ail, assaisonnée à l'huile d'olive", 3.00, 10, 4.7),
        ("Salade Djari", "Salade de mloukhia (corète) aux olives et citron", 3.50, 8, 4.3),
        ("Chlata Felfel", "Salade de poivrons verts frits à l'huile d'olive", 2.50, 8, 4.5),
        ("Felfel Mahchi", "Poivrons farcis à la viande hachée et aux épices", 5.00, 15, 4.6),
        ("Bakbouka", "Salade d'escargots à la menthe et aux épices", 4.00, 12, 4.2),
    ],
    "Plats Principaux": [
        ("Couscous Royal", "Couscous aux légumes de saison, merguez, poulet et mouton", 8.00, 30, 4.9),
        ("Couscous Bel Hout", "Couscous au poisson, légumes et sauce tomate épicée", 9.00, 35, 4.8),
        ("Couscous Tchicha", "Couscous à la tchicha (orge concassée) et légumes", 7.00, 35, 4.6),
        ("Rechta", "Pâtes fraîches algériennes au poulet, pois chiches et navets, sauce blanche parfumée", 7.00, 35, 4.9),
        ("Rechta Mhamsa", "Rechta aux œufs durs et cannelle", 7.50, 30, 4.7),
        ("Chakhchoukha", "Galette émiettée à la sauce tomate, légumes et viande d'agneau, spécialité constantinoise", 7.00, 40, 4.8),
        ("Chakhchoukha Djedj", "Chakhchoukha au poulet et pois chiches", 6.50, 35, 4.6),
        ("Berkoukes", "Gros couscous aux fèves, pois chiches et viande séchée (khlii)", 7.50, 45, 4.7),
        ("Tlitli", "Petites pâtes algériennes au poulet, pois chiches et courgettes", 6.00, 30, 4.6),
        ("Tride", "Crêpes superposées à la sauce blanche et au poulet, arrosées de beurre fondu", 7.00, 40, 4.7),
        ("Lham Lahlou", "Viande d'agneau sucrée aux pruneaux, abricots secs et amandes", 8.50, 45, 4.8),
        ("Tadjine Zitoune", "Tadjine de poulet aux olives vertes et citron confit", 7.00, 30, 4.8),
        ("M'chemmel", "Poulet rôti aux olives, citron confit et épices douces", 7.50, 35, 4.7),
        ("Chtitha", "Viande d'agneau cuite lentement dans une sauce aux herbes et à l'huile d'olive", 8.00, 50, 4.5),
        ("Marqa Bel Assfour", "Ragoût de viande aux herbes fraîches, petits pois et artichauts", 7.00, 40, 4.6),
        ("Dolma", "Légumes farcis (courgettes, poivrons, tomates) à la viande hachée et au riz", 6.50, 35, 4.9),
        ("Mechoui", "Agneau rôti à la broche, servi avec du sel, du cumin et du pain traditionnel", 10.00, 60, 4.9),
        ("Merguez", "Saucisses épicées grillées, servies avec frites et sauce harissa", 5.00, 12, 4.7),
        ("Mderbel", "Purée de courgettes à la viande hachée et aux épices", 5.50, 20, 4.4),
        ("Chouikh", "Ragoût de viande séchée (khlii) aux oignons et tomates", 6.00, 25, 4.3),
    ],
    "Spécialités Régionales": [
        ("Couscous Kabyle (Seksou)", "Couscous aux fèves, petits pois, olives et poulet, spécialité de Kabylie", 7.50, 35, 4.8),
        ("Aghroum n'Tchicha", "Pain d'orge kabyle aux légumes et à l'huile d'olive", 5.00, 25, 4.5),
        ("Rfis", "Semoule grillée au beurre et au miel, spécialité des Aurès", 4.00, 15, 4.4),
        ("Couscous Tlemcénien", "Couscous fin aux raisins secs, cannelle et amandes", 8.00, 35, 4.7),
        ("Djari (Mloukhia)", "Sauce verte à la corète servie avec de la viande et du pain, spécialité annabie", 7.00, 50, 4.6),
        ("Taguella", "Pain cuit sous les cendres, spécialité saharienne, servi avec du thé", 3.50, 20, 4.5),
        ("Mhancha Kabyle", "Galette farcie aux oignons, pommes de terre et herbes", 4.00, 15, 4.6),
    ],
    "Pâtisseries": [
        ("Baklawa", "Feuilleté aux amandes et au miel, parfumé à l'eau de fleur d'oranger", 3.00, 5, 4.9),
        ("Makroud", "Gâteau de semoule aux dattes, frit et trempé dans le miel", 2.50, 5, 4.8),
        ("Griwech", "Pâtisserie frite en forme de rosace, au miel et au sésame", 2.50, 8, 4.7),
        ("Zlabia", "Beignet au miel de forme torsadée, spécialité ramadhanesque", 2.00, 5, 4.6),
        ...
        ("Tcharek", "Cornes de gazelle farcies aux amandes et à la fleur d'oranger", 3.00, 5, 4.8),
        ("Millefuit Algérien", "Millefeuille à la crème pâtissière parfumée à la fleur d'oranger", 3.50, 5, 4.7),
    ],
    "Desserts": [
        ("Baghrir", "Crêpe mille trous à la semoule, servie avec du miel et du beurre", 2.50, 8, 4.8),
        ("Msemen", "Crêpe feuilletée algérienne, servie nature ou au miel", 2.00, 8, 4.7),
        ("Mesfouf", "Couscous sucré aux raisins secs, beurre et cannelle", 3.00, 10, 4.6),
        ("Chrik", "Pain au beurre parfumé à la fleur d'oranger et au miel", 2.50, 10, 4.5),
        ("Lablabi", "Pudding aux pois chiches et au miel", 2.50, 5, 4.3),
    ],
    "Boissons Chaudes": [
        ("Thé à la Menthe", "Thé vert préparé à la menthe fraîche, servi avec des pignons de pin", 1.50, 5, 4.9),
        ("Thé Aux Herbes", "Infusion de verveine, menthe et thym du Djurdjura", 1.50, 5, 4.5),
        ...
        ("Hbib", "Infusion de plantes du sud algérien, légèrement sucrée", 2.50, 7, 4.3),
    ],
    "Boissons Froides": [
        ...
        ("Cherbet", "Sirop traditionnel aux fruits et à l'eau de fleur d'oranger", 2.00, 3, 4.5),
    ],
}
...
INVENTORY = [
    ...
    ("Semoule Fine", "kg", 10, 1.50, "Minoterie d'Alger", "Semoule de blé dur fine pour couscous"),
    ("Semoule Moyenne", "kg", 15, 1.50, "Minoterie d'Alger", "Semoule moyenne pour msemen et baghrir"),
    ("Semoule Grosse", "kg", 20, 1.20, "Minoterie des Aurès", "Grosse semoule pour couscous royal"),
    ("Farine", "kg", 20, 0.80, "Minoterie d'Alger", "Farine de blé tout usage"),
    ("Huile d'Olive", "litre", 10, 8.00, "Huilerie de Kabylie", "Huile d'olive extra vierge"),
    ("Beurre", "kg", 8, 5.00, "Ferme Laitière de Blida", "Beurre frais"),
    ("Viande d'Agneau", "kg", 15, 14.00, "Boucherie Halal Centrale", "Viande d'agneau élevé en plein air"),
    ("Viande de Bœuf", "kg", 12, 12.00, "Boucherie Halal Centrale", "Viande de bœuf locale"),
    ("Poulet", "kg", 18, 6.00, "Ferme Avicole de l'Est", "Poulet fermier élevé au grain"),
    ("Merguez", "kg", 10, 8.00, "Boucherie Traditionnelle", "Saucisses épicées algériennes"),
    ("Poisson Merlan", "kg", 6, 10.00, "Pêcherie d'Alger", "Merlan frais de la Méditerranée"),
    ("Poitrine d'Agneau Séchée", "kg", 3, 18.00, "Boucherie Traditionnelle", "Khlii (viande séchée) traditionnel"),
    ("Pois Chiches", "kg", 15, 2.50, "Épices et Légumineuses Bab El Oued", "Pois chiches secs"),
    ("Lentilles", "kg", 10, 2.00, "Épices et Légumineuses Bab El Oued", "Lentilles brunes"),
    ("Fèves Sèches", "kg", 12, 2.50, "Épices et Légumineuses Bab El Oued", "Fèves décortiquées"),
    ("Fèves Fraîches", "kg", 5, 3.00, "Marché de la Lyre", "Fèves fraîches de saison"),
    ("Tomates", "kg", 25, 1.50, "Marché de la Lyre", "Tomates fraîches"),
    ("Tomates Pelées", "boîte", 20, 1.00, "Conserverie d'Alger", "Tomates pelées en conserve"),
    ("Concentré de Tomate", "kg", 10, 2.00, "Conserverie d'Alger", "Double concentré de tomate"),
    ...
    ("Olives Vertes", "kg", 8, 4.00, "Conserverie de Kabylie", "Olives vertes confites"),
    ("Citrons Confits", "kg", 5, 6.00, "Conserverie de Kabylie", "Citrons confits au sel"),
    ("Dattes Deglet Nour", "kg", 10, 8.00, "Palmeraie de Tolga", "Dattes Deglet Nour de Tolga"),
    ...
    ("Menthe Fraîche", "botte", 15, 1.00, "Marché de la Lyre", "Menthe fraîche pour le thé"),
    ...
    ("Eau de Fleur d'Oranger", "litre", 5, 5.00, "Distillerie de Blida", "Eau de fleur d'oranger naturelle"),
    ("Vermicelle", "kg", 8, 1.50, "Pâtes d'Algérie", "Vermicelle fine"),
    ("Pâte Feuilletée (Brick)", "paquet", 20, 2.00, "Pâtes d'Algérie", "Feuilles de brick traditionnelles"),
    ("Œufs", "unité", 60, 0.20, "Ferme Avicole de l'Est", "Œufs de poules élevées en plein air"),
...
def past_time(days_ago: int, h: int = 12, m: int = 0) -> datetime:
    return (NOW - timedelta(days=days_ago)).replace(hour=h, minute=m, second=0, microsecond=0)
...
def seed_postgres():
    ...
    with engine.begin() as conn:
        ...
        existing = conn.execute(t_resto.select().limit(1)).fetchone()
        if existing:
            logger.info("PostgreSQL already seeded, skipping.")
            return
        logger.info("No existing data found, seeding...")

        # ── Platform Settings ────
        conn.execute(t_settings.insert().values(
            id=1,
            currency="DZD",
        ...
        resto_ids = [row[0] for row in conn.execute(t_resto.select().with_only_columns(t_resto.c.id).order_by(t_resto.c.id)).fetchall()]
        ...
        for ri in range(4):
            for si in range(4):
                s = USERS["staff"][ri * 2 + si % 4] if ri * 2 + si % 4 < 8 else USERS["staff"][si]
                idx = ri * 4 + si
                s = USERS["staff"][idx % 8]
                email = f"{s['firstName'].lower()}.{s['lastName'].lower()}.r{ri}@mayda.dz"
                user_rows.append({
                    "email": email, "phone": s["phone"] + ri * 10 + si,
                    "firstName": f"{s['firstName']}", "lastName": f"{s['lastName']}",
                    "role": staff_roles[si], "isActive": True,
        ...
        user_rows_all = conn.execute(
            t_user.select().with_only_columns(t_user.c.id, t_user.c.role, t_user.c.email).order_by(t_user.c.id)
        ).fetchall()
        ...
        for i, cu in enumerate(client_users):
            ...
            conn.execute(t_addr.insert().values(
                userId=cu.id, street=addr["street"], city=addr["city"],
        ...
        for ri in range(4):
            for i in range(1, 13):
                ...
                conn.execute(t_table.insert().values(
                    restaurantId=resto_ids[ri],
        ...
        for ri in range(4):
            ...
            conn.execute(t_menu.insert().values(
                restaurantId=resto_ids[ri],
            ...
            mid = conn.execute(
                t_menu.select().with_only_columns(t_menu.c.id)
            ...
            for ci, (cn, cd, _) in enumerate(MENU_CATEGORIES):
                if cn in DISHES:
                    conn.execute(t_cat.insert().values(
                        menuId=mid, name=cn, description=cd,
                        isActive=True, displayOrder=ci + 1,
                        createdAt=NOW, updatedAt=NOW,
                    ))
                    cat_id = conn.execute(
                        t_cat.select().with_only_columns(t_cat.c.id)
                        .where(t_cat.c.menuId == mid).where(t_cat.c.name == cn)
                    ).scalar()
                    ...
                    for di, dish_data in enumerate(DISHES[cn]):
                        ...
                        conn.execute(t_dish.insert().values(
                            categoryId=cat_id,
                            name=name, description=desc,
                            price=price, isAvailable=True, quantity=qty,
                            preparationTime=prep, popularity=popular,
                            displayOrder=di + 1,
                            createdAt=NOW, updatedAt=NOW,
                        ))
                        dish_id = conn.execute(
                            t_dish.select().with_only_columns(t_dish.c.id)
                            .where(t_dish.c.categoryId == cat_id).where(t_dish.c.name == name)
                        ).scalar()
                        ...
                        inv_keywords = {
                            "Couscous": ["Semoule Grosse", "Huile d'Olive", "Pois Chiches", "Carottes", "Courgettes", "Tomates", "Oignons", "Navets"],
                            "Chorba": ["Vermicelle", "Tomates", "Oignons", "Pois Chiches", "Coriandre Fraîche", "Cumin", "Viande d'Agneau"],
                            "Harira": ["Tomates", "Lentilles", "Pois Chiches", "Coriandre Fraîche", "Cumin", "Concentré de Tomate"],
                            "Bourek": ["Pâte Feuilletée (Brick)", "Viande de Bœuf", "Oignons", "Persil Frais", "Cumin"],
                            "Mhadjeb": ["Semoule Fine", "Tomates", "Oignons", "Poivrons Verts", "Huile d'Olive"],
                            "Rechta": ["Semoule Fine", "Poulet", "Pois Chiches", "Navets", "Cannelle"],
                            "Chakhchoukha": ["Farine", "Tomates", "Poivrons Verts", "Viande d'Agneau", "Pois Chiches", "Ras El Hanout"],
                            "Berkoukes": ["Semoule Grosse", "Fèves Sèches", "Pois Chiches", "Viande d'Agneau", "Tomates"],
                            "Baklawa": ["Amandes", "Miel", "Eau de Fleur d'Oranger", "Beurre"],
                            "Makroud": ["Semoule Fine", "Dattes", "Miel", "Huile d'Olive"],
                            "Baghrir": ["Semoule Fine", "Miel", "Beurre", "Eau de Fleur d'Oranger"],
                            "Thé": ["Menthe Fraîche"],
                            "Tadjine": ["Viande d'Agneau", "Olives Vertes", "Citrons Confits", "Huile d'Olive", "Safran"],
                            "Dolma": ["Viande de Bœuf", "Riz", "Courgettes", "Poivrons Verts", "Tomates"],
                            "Mechoui": ["Viande d'Agneau", "Cumin", "Sel"],
                            "Merguez": ["Merguez", "Harissa"],
                            "Lham": ["Viande d'Agneau", "Pruneaux", "Amandes", "Cannnelle"],
                            "Msemen": ["Semoule Fine", "Beurre", "Huile d'Olive"],
                            "Sfenj": ["Farine", "Huile d'Olive"],
                            "Lben": ["Lait"],
                            "Tlitli": ["Semoule Fine", "Poulet", "Pois Chiches", "Courgettes", "Cannelle"],
                            "Zlabia": ["Farine", "Miel", "Eau de Fleur d'Oranger"],
                            "Griwech": ["Semoule Fine", "Beurre", "Miel"],
                            "Tride": ["Farine", "Poulet", "Beurre", "Cannelle"],
                            "M'chemmel": ["Poulet", "Olives Vertes", "Citrons Confits", "Huile d'Olive"],
                            "Djari": ["Coriandre Fraîche", "Huile d'Olive", "Viande de Bœuf"],
                            "Taguella": ["Farine", "Sel"],
                            "Karantika": ["Pois Chiches", "Œufs", "Cumin"],
                            "Chtitha": ["Viande d'Agneau", "Persil Frais", "Huile d'Olive"],
                            "Chouikh": ["Poitrine d'Agneau Séchée", "Oignons", "Tomates"],
                            "Mderbel": ["Courgettes", "Viande de Bœuf", "Tomates"],
                            "Chourba": ["Poulet", "Vermicelle", "Tomates", "Carottes", "Cannelle"],
                            "Chrik": ["Farine", "Beurre", "Miel", "Eau de Fleur d'Oranger"],
                            "Mesfouf": ["Semoule Grosse", "Raisins Secs", "Beurre", "Cannelle"],
                            "Rfis": ["Semoule Fine", "Beurre", "Miel", "Dattes"],
                            "M'kacher": ["Semoule Fine", "Beurre", "Miel"],
                            "Mhancha": ["Semoule Fine", "Oignons", "Pommes de Terre", "Huile d'Olive"],
                            ...
                            "Salade Mechouia": ["Poivrons Verts", "Tomates", "Ail", "Huile d'Olive"],
                        }
        ...
        for ri in range(4):
            for inv_item in INVENTORY:
                ...
                conn.execute(t_inv.insert().values(
                    restaurantId=resto_ids[ri],
                    name=iname, description=idesc, unit=iunit,
                    currentStock=stock, minimumStock=imin,
                    unitPrice=icost, supplier=isupplier,
                    isActive=True, createdAt=NOW, updatedAt=NOW,
                ))
                inv_id = conn.execute(
                    t_inv.select().with_only_columns(t_inv.c.id)
                    .where(t_inv.c.restaurantId == resto_ids[ri]).where(t_inv.c.name == iname)
                ).scalar()
                ...
                if dish_row.name in dish_to_inv_map:
                    for ing_name in dish_to_inv_map[dish_row.name]:
                        if ing_name in inv_by_name:
                            ...
                            conn.execute(t_ing_link.insert().values(
                                dishId=dish_row.id,
        ...
        for cu in client_users:
            ...
            conn.execute(t_loyalty.insert().values(
                userId=cu.id, points=points, createdAt=NOW, updatedAt=NOW,
            ))
        ...
        for ci in range(min(4, len(client_users))):
            ...
            if table_result:
                ...
                conn.execute(t_reservation.insert().values(
        ...
        for days_ago in [0, 1, 2, 3, 5, 7, 10, 14, 21, 28]:
            for slot in range(6):
                ...
                selected = random.sample(restaurant_dishes, min(num_items, len(restaurant_dishes)))
                subtotal = sum(d.price for d in selected)
                total = round(subtotal + random.uniform(0, 5), 2)

                timestamp = past_time(days_ago, random.randint(10, 22), random.randint(0, 59))
                ...
                status_val = "COMPLETED" if days_ago > 0 else random.choice(["CONFIRMED", "PREPARING", "READY", "COMPLETED"])
                pay_status = "PAID" if status_val == "COMPLETED" else random.choice(["PENDING", "PAID"])
                order_type = random.choice(["DINE_IN", "TAKEAWAY", "DELIVERY"])
                order_num = f"ORD-{1000 + order_count}"

                conn.execute(t_order.insert().values(
                    ...
                    preparedAt=prepared_at if status_val in ("COMPLETED", "READY", "PREPARING") else None,
                    readyAt=ready_at if status_val in ("COMPLETED", "READY") else None,
                    completedAt=completed_at if status_val == "COMPLETED" else None,
                    createdAt=timestamp,
                    updatedAt=NOW,
                ))
                order_id = conn.execute(
                    t_order.select().with_only_columns(t_order.c.id)
                    .where(t_order.c.orderNumber == order_num)
                ).scalar()

                for dish_row in selected:
                    qty = random.randint(1, 3)
                    conn.execute(t_oi.insert().values(
                        orderId=order_id,
                        dishId=dish_row.id,
                        quantity=qty,
                ...
                if status_val == "COMPLETED":
                    ...
                    conn.execute(t_review.insert().values(
                        userId=cu.id,
                        restaurantId=resto_ids[ri],
                        dishId=chosen.id,
...
def seed_sqlite():
    ...
    if not os.path.exists(AI_SQLITE_PATH):
        logger.warning("SQLite file not found at %s. The AI service may not have started yet.", AI_SQLITE_PATH)
        return
    ...
    if count > 50:
        logger.info("SQLite already seeded (%d food items), skipping.", count)
        conn.close()
        return

    logger.info("Clearing existing AI data...")
    cursor.execute("DELETE FROM food_items")

    ALGERIAN_FOOD_ITEMS = [
        # (name, category, unit, description)
        ("Semoule", "Céréales", "kg", "Semoule de blé dur pour couscous et pâtisseries"),
        ...
        ("Khlii (Viande Séchée)", "Viande", "kg", "Viande d'agneau séchée traditionnelle"),
    ...
    conn.commit()
    conn.close()
    logger.info(f"AI SQLite seeded with {len(ALGERIAN_FOOD_ITEMS)} Algerian food items ✅")
